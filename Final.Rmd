...
lm.subset <- glm(COST ~ RXPM + GS + AGE + RI2 + MM2 + RI + MM, data=newdata.train)
summary(lm.subset)

cv.err.st <- cv.glm(newdata.train,lm.subset)
cv.err.st$delta[1]
...
treemodel.test=newdata[-train ,"COST"]
plot(yhat.1,treemodel.test)
mean((yhat.1-treemodel.test)^2)
```
