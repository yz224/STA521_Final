...
=======
Try transformations
>>>>>>> 542bc48fd45dc11b9ad460c195bf0be9f16ae67c

```{r}
lapply(1:8,function(x) hist(data[,x], main=names(data)[x],xlab=names(data)[x]))
```

From the histogram of each variable, we can see that some varialbes don't follow normal distribution. We want to find out the best transformation of each variable.


Using unconditional box cox transformations for independent variable. Then, transform response accordingly. 

```{r}
powerTransform(data)$round
summary(powerTransform(data))
test=data
test$GS=test$GS^2
test$RI=sqrt(test$RI)
test$COPAY=log(test$COPAY)
test$MM=sqrt(test$MM)
summary(powerTransform(lm(COST~., test)))
summary(powerTransform(test))
