...
BMA <- MC3.REG(all.y = test$COST, 
        all.x = matrix(c(test$RXPM,test$GS,test$RI,test$COPAY,test$AGE,test$F,test$MM),nrow=29),
...
try gam
```{r}
gam.1 <- glm(COST ~ RXPM + GS + poly(RI,4) + COPAY + AGE + F + MM,data=test)
summary(gam.1)
anova(lm.1,gam.1)
cv.error=rep(0,5)
for (i in 1:5){
  glm.fit=glm(COST ~ RXPM + GS + poly(RI,i) + COPAY + AGE + F + MM,data=test)
  cv.error[i]=cv.glm(test,glm.fit)$delta[1]
  }
cv.error
```
...
lasso.coef
lasso.pred <- predict(out,s=bestlam,x[-train,])
lasso.pred
...
```{r}
regfit.fwd <- regsubsets(COST~.,data=test,nvmax=7,method="forward")
summary(regfit.fwd)
regfit.bwd <- regsubsets(COST~.,data=test,nvmax=7,method="backward")
summary(regfit.bwd)
```
